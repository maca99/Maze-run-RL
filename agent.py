...
class agent:
    ...
    ##Azioni vietate
    up_forbidden = [(1,1),(1,3),(2,1),(2,2),(3,1),(3,2),(8,1),(8,3),(9,3)] #che non siano y=4 

    down_forbiden = [(1,2),(1,4),(2,2),(2,3),(2,4),(3,2),(3,3),(8,2),(8,4),(9,4)] #che non siano y=1

    right_forbiden = [(1,3),(7,2),(7,3)] #che non siano y=4,y=5e y=9

    left_forbiden = [(2,3),(8,2),(8,3)] #che non siano y=1|5|6

    ## ACTIONS  
    def action_up(state):
        if(state.x == 4 or (state.x,state.y) in env.up_forbidden):
            return state
        else:
            return env.build_state(state.x+1,state.y)
      
    def action_down(state):
        if(state.x == 1 or (state.x,state.y) in env.down_forbidden):
            return state
        else:
            return env.build_state(state.x-1,state.y)
    
    def action_right(state):
        if(state.y == 4,5,6 or (state.x,state.y) in env.right_forbidden):
            return state
        else:
            return env.build_state(state.x,state.y+1)
        
    def action_left(state):
        if(state.y == 1,5,6 or (state.x,state.y) in env.left_forbidden):
            return state
        else:
            return env.build_state(state.x,state.y-1)
    ...
    actions = {'up': action_up, 'down': action_down, 'left': action_left, 'right': action_right}
